...
async function renderNigredoArchive() {
    const container = document.getElementById('m-nigredo-archive-list');
    if (!container) return;
    ...
    try {
        ...
        archiveData.forEach(item => {
            ...
            dlBtn.onclick = (e) => {
                ...
                dlBtn.onclick = (e) => {
                    e.preventDefault();
                    window.RitualDownload(`/api/grimoire/download/nigredo/${item.idx}`, `${displayName}.xlsx`);
            };
            ...
            delBtn.onclick = async (e) => {
                e.preventDefault();
                if (confirm(`Erase the entire Nigredo archive for '${displayName}'?`)) {
                    try {
                        const delRes = await fetch(`/api/grimoire/delete/nigredo/${item.idx}`, { method: 'DELETE' });
                        if (delRes.ok) renderNigredoArchive(); 
