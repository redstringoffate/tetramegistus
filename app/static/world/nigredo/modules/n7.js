/* static/world/nigredo/modules/n7.js */
...
function getN7MateriaClass(name) {
    ...
    if (N7_BODIES.lilith.includes(name)) return 'p-node';
...
function getN7LordClasses(planetName, chartInfo) {
    if (!chartInfo) return '';
...
function updateN7UI() {
    document.querySelectorAll('#n7-system-nav .n7-sys-tab').forEach(btn => 
        btn.classList.toggle('active', btn.dataset.sys === N7_STATE.system)
    );
    ...
    if (vault) {
        if (N7_STATE.system === 'sidereal') {
            ...
            document.querySelectorAll('#sidereal-controls-panel-n7 .n7-ayan-tab').forEach(btn => 
                btn.classList.toggle('active', btn.dataset.ayan === N7_STATE.ayanamsa)
            );
        } else { vault.style.visibility = 'hidden'; }
    }

    document.querySelectorAll('#n7-category-tabs .n7-ayan-tab').forEach(btn => 
        btn.classList.toggle('active', btn.dataset.cat === N7_STATE.category)
    );
    
    const hermeticBtn = document.getElementById('btn-cat-hermetic-n7');
    if (hermeticBtn) hermeticBtn.style.display = (N7_STATE.system === 'tropical') ? 'inline-block' : 'none';
    ...
    document.getElementById('n7-view-sigilum').style.display = 'none';
    document.getElementById('n7-view-plenitudo-sabian').style.display = 'none';
    document.getElementById('n7-view-plenitudo-tabula').style.display = 'none';

    if (N7_STATE.mode1 === 'sigilum') {
        document.getElementById('n7-view-sigilum').style.display = 'block';
        renderSigilumSkeleton();
    } else if (N7_STATE.mode1 === 'plenitudo' && N7_STATE.mode2 === 'sabian') {
        document.getElementById('n7-view-plenitudo-sabian').style.display = 'block';
        renderPlenitudoSabianSkeleton();
    } else {
        document.getElementById('n7-view-plenitudo-tabula').style.display = 'block';
...
async function fetchAndRenderN7() {
    ...
    const { system, ayanamsa, category } = N7_STATE;
    N7_STATE.data = null; 
    updateN7UI(); 

    try {
        const url = `/api/astro/hypostases/reading?category=${category}&system=${system}&ayanamsa=${ayanamsa}&h_sys=${h_sys}`;
        const res = await fetch(url);
        const resData = await res.json();
        if (resData.error) return;
        N7_STATE.data = resData.data; 
        populateN7Data();
    } catch (e) { console.error("[N7] Fetch failed", e); }
}
...
function getN7ElementGlow(text) {
    if (!text) return '';
...
function formatLordsGlow(lordStr) {
    ...
    return lordStr.split('|').map(p => {
        const trimP = p.trim();
        return `<span class="${getN7PlanetGlow(trimP)}">${trimP}</span>`;
    }).join(' | ');
}

// 🚀 [수정] Sigilum 렌더링 함수 (CSS 클래스 동적 주입)
function populateN7Sigilum() {
    const tbody = document.getElementById('n7-tbody-sigilum');
    if (!tbody) return;
    tbody.innerHTML = '';
    ...
    targets.forEach(t => {
        ...
        tr.innerHTML = `
            <td class="n7-hover-target" data-tooltip="${tooltipStr}" style="cursor:help;">
                <strong class="${getN7MateriaClass(t)}">${t.toUpperCase()}</strong>
            </td>
            <td style="color:#aaa;"><span class="${getN7ZodiacGlow(dmsText)}">${dmsText}</span></td>
            ...
            <td style="text-align:center !important;"><span class="${getN7ElementGlow(pData.duad)}">${pData.duad || '-'}</span></td>
            <td style="text-align:center !important;"><span class="${getN7ElementGlow(pData.dodeca)}">${pData.dodeca || '-'}</span></td>
            <td style="text-align:center !important;"><span class="${getN7PlanetGlow(pData.decan)}">${pData.decan || '-'}</span></td>
            <td style="text-align:center !important;"><span class="${getN7PlanetGlow(pData.bound)}">${pData.bound || '-'}</span></td>
            <td class="col-sabian-symbol" data-sabian="${sabianIdx}" style="color:#aaa; font-style:italic;">...</td>
        `;
        tbody.appendChild(tr);
    });
    renderN7SabianSymbolsInView('#n7-tbody-sigilum');
}

// 🚀 [수정]: Sabian Matrix 렌더링 함수 (Hover 필터링, 대문자 변환, 볼드 처리)
function populateN7Sabian() {
    const targets = N7_BODIES[N7_STATE.category] || [];
    const tbody = document.getElementById('n7-tbody-sabian');
    const theadTr = document.querySelector('#n7-thead-sabian tr');
    if(!tbody) return;
    ...
    PLENITUDO_LEFT_COL.forEach(rowItem => {
        ...
        if (rowItem === "DIVIDER") {
            ...
            let rowHTML = `<td><strong class="${getN7MateriaClass(rowItem)}">${rowItem}</strong></td>`;
            targets.forEach((colTargetName, idx) => {
                const chartData = N7_STATE.data[colTargetName];
                let cellHTML = `<div class="n7-cell-line1" style="color:#555;">-</div><div class="n7-cell-line2">-</div>`;
                ...
                if(chartData) {
                    if(theadTr) {
                        ...
                        if(th) {
                            const info = chartData.chart_info;
                            th.classList.add('n7-hover-target');
                    ...
                    if(pData && pData.dms) {
                        ...
                        if (isIntersection) cellClass = "n7-cell-intersection";
                        ...
                        if (tooltip) hoverClass = "n7-hover-target";
                        ...
                        cellHTML = `
                            <div class="n7-cell-line1" style="font-weight: ${fontWeight};">${dmsDisp} | <strong style="color:#fff;">${pData.house || '-'}</strong></div>
                            <div class="n7-cell-line2 col-sabian-symbol" data-sabian="${sIdx}">...</div>
                        `;
                    }
                }
                ...
                rowHTML += `<td class="n7-sabian-cell ${hoverClass} ${cellClass}" ${tooltipAttr} ${cursorStyle}>${cellHTML}</td>`;
            });
            tr.innerHTML = rowHTML;
        }
        tbody.appendChild(tr);
    });
    renderN7SabianSymbolsInView('#n7-tbody-sabian');
}
...
function populateN7Tabula() {
    const targets = N7_BODIES[N7_STATE.category] || [];
    const tbody = document.getElementById('n7-tabula-body');
    const theadTr = document.getElementById('n7-tabula-head-row');
    if(!tbody) return;
    ...
    targets.forEach((targetName, colIdx) => {
        ...
        for (const [bodyName, pData] of Object.entries(chartData.points)) {
            ...
            if ((N7_STATE.system === 'draconic' || N7_STATE.system === 'ketunic') && 
            ...
            if(tr && tr.cells[cellIndex]) {
                ...
                const lordClasses = getN7LordClasses(bodyName, chartData.chart_info);
                marker.className = `${getN7MateriaClass(bodyName)} ${lordClasses}`.trim();
                
                marker.classList.add('n7-hover-target');
            ...
            for (let i = 1; i <= 12; i++) {
                ...
                if (hLon !== undefined) {
                    ...
                    if (tr && tr.cells[cellIndex]) {
                        ...
                        marker.className = `n7-house-marker p-cusp n7-hover-target`;
    ...
    // 🚀 [DOM 물리적 재정렬 방역]: 표에 다 때려박은 후, 한 칸(Cell)에 2개 이상 들어있으면 소수점 기준으로 위치 강제 변경!
    Array.from(tbody.rows).forEach(tr => {
        for(let c = 1; c <= targets.length; c++) {
            const cell = tr.cells[c];
            // 해당 셀 안의 마커(div)들을 배열로 수집 (선/배경 제외하고 exact-lon 있는 요소만)
...
} // populateN7Tabula() 닫히는 곳

/* 5. HELPERS */
function renderSigilumSkeleton() {
    const tbody = document.getElementById('n7-tbody-sigilum');
    if (!tbody) return;
    if (!N7_STATE.data) tbody.innerHTML = `<tr><td colspan="11" class="text-center" style="padding:30px; color:#555; font-style:italic;">Loading Personas...</td></tr>`;
    else populateN7Sigilum();
}

function renderPlenitudoSabianSkeleton() {
    const thead = document.getElementById('n7-thead-sabian');
    const tbody = document.getElementById('n7-tbody-sabian');
    const targets = N7_BODIES[N7_STATE.category] || [];
    let headHTML = `<tr><th class="n7-col-header">CELESTIAL BODIES</th>`;
    targets.forEach(t => headHTML += `<th class="n7-col-header n7-hover-target ${getN7MateriaClass(t)}">${t}</th>`);
    thead.innerHTML = headHTML + `</tr>`;
    if(!N7_STATE.data) tbody.innerHTML = `<tr><td colspan="${targets.length + 1}" style="text-align:center; padding:30px; color:#555;">Loading Matrix...</td></tr>`;
    else populateN7Sabian();
}

function renderPlenitudoTabulaSkeleton() {
    const wrapper = document.getElementById('n7-tabula-wrapper');
    wrapper.innerHTML = `<div class="codex-table-wrapper" style="width:100%; height:75vh; overflow:auto;"><table class="n8-table n7-tabula-table"><thead><tr id="n7-tabula-head-row"></tr></thead><tbody id="n7-tabula-body"></tbody></table></div>`;
    const thead = document.getElementById('n7-tabula-head-row');
    const tbody = document.getElementById('n7-tabula-body');
    const targets = N7_BODIES[N7_STATE.category] || [];
    let headHTML = `<th class="sticky-col">SABIAN NUMBER</th>`;
    targets.forEach(t => headHTML += `<th class="${getN7MateriaClass(t)}">${t.toUpperCase()}</th>`);
    ...
    renderN7SabianSymbolsInView('#n7-tabula-body');
}
...
async function fetchN7SabianSymbols() {
    ...
    } catch (e) { console.warn("[N7] Sabian Fail", e); }
}

async function renderN7SabianSymbolsInView(containerSelector) {
    if (!N7_STATE.sabianSymbols) await fetchN7SabianSymbols();
    if (!N7_STATE.sabianSymbols) return;
    const syms = N7_STATE.sabianSymbols;
...
function bindN7Tooltips() {
    const tooltip = document.getElementById('n7-info-tooltip');
    if (!tooltip) return;
    document.addEventListener('mouseover', e => {
        const target = e.target.closest('.n7-hover-target');
    ...
    document.addEventListener('mouseout', e => {
        if (e.target.closest('.n7-hover-target')) tooltip.style.display = 'none';
    });
}

/* ─────────────────────────────────────────────────────────────
   X. GRIMOIRE MANIFESTATION (N7 -> Archive)
   ───────────────────────────────────────────────────────────── */
window.saveToGrimoire = async function() {
    ...
    // 3. 다이나믹 컴파일러 라우팅 (Absolute Structure)
    ...
    if (m1 === 'sigilum') {
        ...
        if (m2 === 'sabian') {
            compilerId = `n7_sabian_${cat}`;
        } else if (m2 === 'tabula') {
            compilerId = `n7_${cat}_${currentLang}`;
        }
    }

    if (!compilerId) {
        console.error("[GRIMOIRE] Invalid N7 state for compilation.", {m1, m2, cat});
        alert("System State Error: Cannot determine compiler.");
        return false;
    }

    // 4. Payload 조립 (Python N7 컴파일러가 읽을 수 있도록 seed 전체 전송)
    const payload = {
        seed_id: seedId,        
        stage: 'nigredo',       
