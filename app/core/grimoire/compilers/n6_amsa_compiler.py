...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/n6_amsa_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/nigredo/n6_amsa.xlsx'))
...
def compile_n6_amsa_grimoire(chart_data, seed_data=None):
    ...
    if ayan_ref:
        ...
        old_size = a_cell.font.size if a_cell.font else 11
        a_cell.font = Font(name="Consolas", size=old_size, bold=False, color=color_to_use)
    ...
    if amsa_ref:
        ...
        old_size = am_cell.font.size if am_cell.font else 11
        am_cell.font = Font(name="Consolas", size=old_size, bold=True, color=color_to_use)
        am_cell.alignment = Alignment(horizontal='right', vertical='center')

    # 🚀 4. Amsa Name (C5) - 🌟 우측 정렬로 수정됨
    ...
    if name_ref:
        ...
        old_size = n_cell.font.size if n_cell.font else 11
        n_cell.font = Font(name="Consolas", size=old_size, bold=False, color=color_to_use)
        n_cell.alignment = Alignment(horizontal='right', vertical='center') # 👈 우측 정렬!
    ...
    forced_widths = { "A": 21.0, "B": 25.0, "C": 25.0 }
