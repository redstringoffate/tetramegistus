...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/n5_unus_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/nigredo/n5_unus.xlsx'))

# 🚀 [업데이트됨]: 요청하신 새로운 렌더링 순서 반영 (Quincunx가 8순위로 이동)
ASPECT_ORDER = {
...
def compile_n5_unus_grimoire(chart_data, seed_data=None):
    ...
    for body_name, row_idx in sorted_bodies:
        ...
        for i, asp_data in enumerate(body_asps):
            ...
            for c_char in [col_b, col_c, col_d, col_e, col_f]:
                ...
                # col_b(애스펙트 이름)는 skip_c가 False이므로 styler.py가 색상을 입혀줍니다!
