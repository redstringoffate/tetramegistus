...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/a3_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/albedo/a3.xlsx'))
...
def compile_a3_grimoire(chart_data, seed_data=None):
    ...
    # 🚀 A3 특징: 이중 스탬프(Composite)와 달리 Davison 데이터를 이용해 'A2' 셀에 단일 스탬프만 찍음!
    ...
    orb_cell = mapping["metadata"].get("fixed_star_orb")
    if orb_cell and ws[orb_cell]:
        ws[orb_cell] = f"{fixed_star_orb:.1f}"
        ws[orb_cell].data_type = 's'
        ws[orb_cell].number_format = '@'
    ...
    for key in ["ascendant", "midheaven", "sys_tab", "h_sys", "fixed_star_orb", "ayanamsa"]:
        ...
        if c_ref and ws[c_ref].value:
            ...
            # 맑은고딕 제거 및 Consolas 지정 (styler에서 설정한 폰트 색상을 유지합니다)
