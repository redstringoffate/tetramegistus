...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/a2_nak_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/albedo/a2_nak.xlsx'))
...
def compile_a2_nak_grimoire(chart_data, seed_data=None):
    if not os.path.exists(TEMPLATE_FILE):
        raise FileNotFoundError(f"Template missing: {TEMPLATE_FILE}")

    # 파일 복사 없이 템플릿을 메모리로 곧바로 로드
    ...
    # ====================================================================
    # ⬇️ 여기서부터는 Albedo 엔진 데이터 추출 로직입니다.
    # ====================================================================
    ...
    # [이하 n2_nak_compiler.py 의 sabian_dict = {} 부터 맨 끝까지 동일]
    ...
    if h_sys_cell:
        full_h_sys = H_SYS_MAP.get(h_sys.upper(), h_sys.upper())
    ...
    for body in all_bodies:
        ...
        # 🚀 [수정]: 컬럼 범위 및 skip_color 로직 최적화
        # n2_nak은 A~J열까지 사용하므로 범위를 좁히고, 지정 열만 색상을 건너뜁니다.
    ...
    # 🚀 [수정]: 리턴 직전, 시트 전체에 Consolas 폰트 일괄 각인
