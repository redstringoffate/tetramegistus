...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/a5_comp_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/albedo/a5_comp.xlsx'))
...
def compile_a5_comp_grimoire(chart_data, seed_data=None):
    ...
    def resolve_seed(d):
        ...
        loc_val = ""
        for k in keys:
            if d.get(k): loc_val = str(d.get(k)).strip(); break
    ...
    apply_natal_stamp(ws, h_a, method="single", cells=["A2"])
    apply_natal_stamp(ws, h_b, method="single", cells=["A3"])
    ...
    res_a = calculate_principia(date_str=h_a['birth_date'], time_str=h_a['birth_time'], lat=h_a['lat'], lng=h_a['lng'], timezone=h_a['timezone'], system=system, ayanamsa=ayanamsa, view=view, h_sys=h_sys, fixed_star_orb=orb_factor, is_time_unknown=h_a['is_time_unknown'])
    res_b = calculate_principia(date_str=h_b['birth_date'], time_str=h_b['birth_time'], lat=h_b['lat'], lng=h_b['lng'], timezone=h_b['timezone'], system=system, ayanamsa=ayanamsa, view=view, h_sys=h_sys, fixed_star_orb=orb_factor, is_time_unknown=h_b['is_time_unknown'])
    
    comp_map = generate_composite_data(res_a, res_b)

    # 🚀 [FIX 1]: Angles Flip 180도 뒤집힘 방지 로직 (A3 Comp 벤치마킹)
    # 단순 중간값이 아닌 검증된 하우스 커스프를 강제 주입합니다.
    ...
    for body_name, row_idx in sorted_bodies:
        ...
        if num_asps == 0:
            ...
            # 🚀 [FIX 2]: col_a(Body) 렌더링 루프에서 완전 제외 (스타일 침범 방지)
        ...
        for i, asp_data in enumerate(body_asps):
            ...
            # 🚀 [FIX 2]: col_a(Body) 렌더링 루프에서 완전 제외
            for c_char in [col_b, col_c, col_d, col_e, col_f]:
    ...
    pat_start = pat_map.get("row_start", 8)
