...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/a2_anamnesis_mapping.json'))
# N2와 동일한 템플릿 사용 (이름만 a2_anamnesis.xlsx)
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/albedo/a2_anamnesis.xlsx'))
...
def compile_a2_anamnesis_grimoire(chart_data, seed_data=None):
    if not os.path.exists(TEMPLATE_FILE):
        raise FileNotFoundError(f"Template missing: {TEMPLATE_FILE}")

    wb = load_workbook(TEMPLATE_FILE)
    base_name = "a2_anamnesis"
    ...
    principia_res = calculate_principia(
        date_str=str(dav_h.get('birth_date', '2000-01-01')).split('T')[0],
        time_str=str(dav_h.get('birth_time', '12:00:00')),
        lat=dav_h.get("lat", 37.5665), lng=dav_h.get("lng", 126.9780), 
        timezone=_ensure_float_tz(dav_h.get("timezone", 9.0), str(dav_h.get('birth_date', '2000-01-01')).split('T')[0]),
    ...
    # 3. 🚀 [CORE LOGIC]: Anamnesis Shift & Original House Overlay
    ...
    for p_key, p_val in planets_data.items():
        ...
        if len(cusps_simple) == 12:
            ...
            for h_num in range(1, 13):
                ...
                if cur < nxt:
                    if cur <= new_lon < nxt: found_house = h_num; break
                else:
                    if cur <= new_lon < 360 or 0 <= new_lon < nxt: found_house = h_num; break
    ...
    for body in all_bodies:
        ...
        # Draconic / Ketunic 시스템 시 Node 강제 삭제
