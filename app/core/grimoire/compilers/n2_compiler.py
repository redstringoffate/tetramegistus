...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/n2_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/nigredo/n2.xlsx'))
...
def compile_n2_grimoire(chart_data, seed_data=None):
    ...
    # 🚀 입력값이 코드('W')가 아닌 풀네임으로 들어와도 Placidus로 튕기지 않게 방어막 구축
    ...
    if system.upper() in ["DRACONIC", "KETUNIC"]:
        # House Sys 행 추가로 범위를 9까지 확장
        for r_idx in range(2, 9):
    ...
    for body in all_bodies:
        ...
        # 🚀 [최종 진화]: 빈칸과 테두리 잔해를 남기지 않고 행 자체를 물리적으로 완전 삭제
        ...
        col_house = get_col(b_map.get("house", "H")) # 🚀 House 추가
    ...
    # 🚀 유저님의 원본 코드에 있던 가장 안전한 순정 폰트 지정 루프 유지!
