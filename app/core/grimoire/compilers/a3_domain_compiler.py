...
MAPPING_FILE = os.path.abspath(os.path.join(BASE_DIR, '../mappings/a3_domain_mapping.json'))
TEMPLATE_FILE = os.path.abspath(os.path.join(BASE_DIR, '../../../templates_excel/albedo/a3_domain.xlsx'))
...
def compile_a3_domain_grimoire(chart_data, seed_data=None):
    ...
    lat, lng = get_safe_float(dav_hydrated.get("lat"), 37.5665), get_safe_float(dav_hydrated.get("lng"), 126.9780)
    tz = _ensure_float_tz(dav_hydrated.get('tz') if dav_hydrated.get('tz') is not None else dav_hydrated.get('timezone', 9.0), date_str)
    is_unk = bool(dav_hydrated.get("is_time_unknown", 0))

    # Davison의 시공간 데이터로 행성 및 아르카나 도출
    ...
    # 🚀 Sys_tab 및 Ayanamsa 대문자 화 + Whole Sign 치환
    ...
    for key in ["ascendant", "midheaven", "sys_tab", "h_sys", "ayanamsa"]:
        ...
        if c_ref and ws[c_ref].value:
            ...
            # 🚀 의도하신 빈 배열 트릭: is_bold = True if key in [] else False -> 그냥 모두 False로 적용!
            is_bold = False
